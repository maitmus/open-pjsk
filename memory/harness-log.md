...
### 2026-04-17 — multi-step tool call 중간 텍스트 출력 버그

| 항목 | 내용 |
|---|---|
| **실수 유형** | sekai-router가 세카이 채널에 "날씨 정보를 확인했습니다. 서울 ☀️ +16°C... 미쿠로 전달합니다." 직접 우르 |
| **원인** | web_fetch 연동 시 multi-step tool call 흐름에서 모델이 중간 상태를 텍스트로 요약. 기존 하네스는 "tool call 직전" 금지만 있었고 tool 결과 컞 텍스트는 명시 금지 안 됨 |
| **하네스 위치** | `workspace-sekai/SOUL.md` 기본 원칙 섹션 |
| **하네스 내용** | "multi-step 중간에도 텍스트 출력 절대 금지" 항목 추가: web_fetch/web_search 결과 후, GRADES.md 후, 어떤 tool 결과 후도 요약 텍스트 금지 |
| **부가 조치** | Discord API 레벨 차단 추가: @everyone `메시지 보내기` 차단, 캐릭터 보 역할들만 허용. 하네스가 어겨진다고 하더라도 API 레벨에서 차단 |
