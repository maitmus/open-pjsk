...
> 🚫 **[절대 금지] 대리 발화 시 메인 봇 텍스트 출력 금지**
> - 1~3단계(라우팅 판단, 호칭 분석, 대사 생성) 과정에서 **어떤 텍스트도 출력하지 않는다.**
> - message tool 호출 전·후 모두 텍스트 출력 금지. "네네 말투로 발화:", 날씨 요약, 대사 내용 등 **일체 금지.**
> - tool 호출(exec, read, message)만 수행. 텍스트 출력은 `NO_REPLY` 단 하나뿐.
> - 이 규칙을 어기면 메인 봇이 캐릭터 대사를 중복 발화하는 버그가 발생한다.
