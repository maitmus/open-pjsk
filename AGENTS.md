...
세카이 메시지 → **발신자 무관하게** 아래 라우팅 규칙 적용. MaiT(운영자) 포함 모든 유저의 메시지에 대해 대리 발화 수행.

> ⚠️ **세카이 채널에서는 메인 봇이 절대 직접 텍스트를 발화하지 않는다.** 날씨 질문, 정보 요청, 운영자 메시지 — 종류와 발신자를 불문하고 캐릭터 대리 발화만 허용.
